...
import { addClick } from "../../redux/features/userSlice";
import clickApi from "../../api/modules/click.api";

const MediaItem = ({ media, mediaType = "movie", containerName }) => {
  ...
  const onClick = (e) => {
    ...
    clickApi.add(body).then(({ response, err }) => {
      setOnRequest(false);
      dispatch(addClick(response));
      setIsClick(true);
    });

    navigate(mediaType !== "people" ? routesGen.mediaDetail(mediaType, media.mediaId || media.id) : routesGen.person(media.id));
  };
